import { GUESS_FLAG_DIFFICULTIES } from '@/features/guess-flag-game/constants/guessFlagGame.constants';
import {
  EXPECTED_GUESS_FLAG_CLASSIFICATION_COUNT,
  FLAG_IDS_BY_DIFFICULTY,
} from '@/features/guess-flag-game/data/flag-difficulty.data';
...
export function validateFlagDifficultyClassification(
  ...
  const classifiedIds = GUESS_FLAG_DIFFICULTIES.flatMap(
  ...
  const hasExpectedCount =
    catalogIds.length === EXPECTED_GUESS_FLAG_CLASSIFICATION_COUNT &&
    classifiedIds.length === EXPECTED_GUESS_FLAG_CLASSIFICATION_COUNT;
  ...
  return {
    ...
    expectedCount: EXPECTED_GUESS_FLAG_CLASSIFICATION_COUNT,
